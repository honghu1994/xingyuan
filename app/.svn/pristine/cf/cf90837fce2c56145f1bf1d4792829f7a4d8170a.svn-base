<template>
  <div class="fixedAssets">
    <header-top>
      <div slot="left">
        <img @click="$router.go(-1)" src="@/assets/img/back.png" />
      </div>
      <div slot="mid">资产信息</div>
    </header-top>
    
    <div class="zstext-content">
    
    
      <div class="zschoose">
        <template>
        <el-select v-model="subordinate" placeholder="请选择" size="large"  class="choose">
        <el-option
          
          v-for="item in companyData"
         :key="item.subordinate"
         :label="item.company_name"
         :value="item.subordinate">
        </el-option>
       </el-select>
      </template>

      </div>
      
    <!-- tab栏切换 -->
      <div class="mytabs zsul">
        <template>
          <el-tabs v-model="activeName"  class="zsmytabs"  >
          <el-tab-pane label="耕地" name="first">
             <ul >

              <li  class="zsli"  @click='toDetail(item.assets_id)' v-for="(item,index) in officeData">
                <h1 class="zsh1 ">{{item.assets_name}}</h1>
                <p class=" zstwo">
                  <span style="marginRight:.1.5rem">{{item.assets_nature}}</span>
                  <span style="color:#0000ff">{{item.assets_category}}</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==0">尚未交易</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==2">交易中</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==3">完成交易</span>
                 
                  <!-- <span style="marginLeft:.2rem">{{item.transaction_status}}</span> -->
                  <span>建筑面积（m²）：{{item.built_area}}</span>
                </p>
                <p class="zs-last zstwo"></p>
                <p class="zsfont">
                  <span style="marginRight:.5rem">{{item.street_name}}</span>
                  <span style="color:#9b8dfe">{{item.company_name}}</span>
                </p>
              </li>
             
            </ul>
           
          
          </el-tab-pane>
         <el-tab-pane label="林地" name="second">
            <ul >

              <li  class="zsli"  @click='toDetail(item.assets_id)' v-for="(item,index) in factoryData">
                <h1 class="zsh1 ">{{item.assets_name}}</h1>
                <p class=" zstwo">
                  <span style="marginRight:.1.5rem">{{item.assets_nature}}</span>
                  <span style="color:#0000ff">{{item.assets_category}}</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==0">尚未交易</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==2">交易中</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==3">完成交易</span>
                 
                  <!-- <span style="marginLeft:.2rem">{{item.transaction_status}}</span> -->
                  <span>建筑面积（m²）：{{item.built_area}}</span>
                </p>
                <p class="zs-last zstwo"></p>
                <p class="zsfont">
                  <span style="marginRight:.5rem">{{item.street_name}}</span>
                  <span style="color:#9b8dfe">{{item.company_name}}</span>
                </p>
              </li>
             
            </ul>
         </el-tab-pane>
         <el-tab-pane label="圆地" name="third">
              <ul >

              <li  class="zsli"  @click='toDetail(item.assets_id)' v-for="(item,index) in storyData">
                <h1 class="zsh1 ">{{item.assets_name}}</h1>
                <p class=" zstwo">
                  <span style="marginRight:.1.5rem">{{item.assets_nature}}</span>
                  <span style="color:#0000ff">{{item.assets_category}}</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==0">尚未交易</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==2">交易中</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==3">完成交易</span>
                 
                  <!-- <span style="marginLeft:.2rem">{{item.transaction_status}}</span> -->
                  <span>建筑面积（m²）：{{item.built_area}}</span>
                </p>
                <p class="zs-last zstwo"></p>
                <p class="zsfont">
                  <span style="marginRight:.5rem">{{item.street_name}}</span>
                  <span style="color:#9b8dfe">{{item.company_name}}</span>
                </p>
              </li>
             
            </ul>
         </el-tab-pane>
          <el-tab-pane label="草地" name="fourth">
                 <ul >

              <li  class="zsli"  @click='toDetail(item.assets_id)' v-for="(item,index) in dormData">
                <h1 class="zsh1 ">{{item.assets_name}}</h1>
                <p class=" zstwo">
                  <span style="marginRight:.1.5rem">{{item.assets_nature}}</span>
                  <span style="color:#0000ff">{{item.assets_category}}</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==0">尚未交易</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==2">交易中</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==3">完成交易</span>
                 
                  <!-- <span style="marginLeft:.2rem">{{item.transaction_status}}</span> -->
                  <span>建筑面积（m²）：{{item.built_area}}</span>
                </p>
                <p class="zs-last zstwo"></p>
                <p class="zsfont">
                  <span style="marginRight:.5rem">{{item.street_name}}</span>
                  <span style="color:#9b8dfe">{{item.company_name}}</span>
                </p>
              </li>
             
            </ul>
          </el-tab-pane>
          <el-tab-pane label="商务用地" name="fifth">
               <ul >

              <li  class="zsli"  @click='toDetail(item.assets_id)' v-for="(item,index) in othersData">
                <h1 class="zsh1 ">{{item.assets_name}}</h1>
                <p class=" zstwo">
                  <span style="marginRight:.1.5rem">{{item.assets_nature}}</span>
                  <span style="color:#0000ff">{{item.assets_category}}</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==0">尚未交易</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==2">交易中</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==3">完成交易</span>
                 
                  <!-- <span style="marginLeft:.2rem">{{item.transaction_status}}</span> -->
                  <span>建筑面积（m²）：{{item.built_area}}</span>
                </p>
                <p class="zs-last zstwo"></p>
                <p  class="zsfont">
                  <span style="marginRight:.5rem">{{item.street_name}}</span>
                  <span style="color:#9b8dfe">{{item.company_name}}</span>
                </p>
              </li>
             
            </ul>
          </el-tab-pane>
           <el-tab-pane label="住宅用地" name="sixth">
               <ul >

              <li  class="zsli"  @click='toDetail(item.assets_id)' v-for="(item,index) in houseData">
                <h1 class="zsh1 ">{{item.assets_name}}</h1>
                <p class=" zstwo">
                  <span style="marginRight:.1.5rem">{{item.assets_nature}}</span>
                  <span style="color:#0000ff">{{item.assets_category}}</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==0">尚未交易</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==2">交易中</span>
                  <span style="marginLeft:.2rem" v-if="item.transaction_status==3">完成交易</span>
                 
                  <!-- <span style="marginLeft:.2rem">{{item.transaction_status}}</span> -->
                  <span>建筑面积（m²）：{{item.built_area}}</span>
                </p>
                <p class="zs-last zstwo"></p>
                <p  class="zsfont">
                  <span style="marginRight:.5rem">{{item.street_name}}</span>
                  <span style="color:#9b8dfe">{{item.company_name}}</span>
                </p>
              </li>
             
            </ul>
          </el-tab-pane>
         </el-tabs>
      </template>

       
      </div>
    </div>
  </div>
</template>

<script>
import HeaderTop from "@/common/Header";

export default {
  data() {
    return {
      houseData:[],
      othersData:[],
      dormData:[],
      storyData:[],
        factoryData:[],
        companyData:[],
        officeData:[],
        activeName: 'first',
        subordinate:'D440305101002',
         id:this.$store.state.loginInfo.sessionVal.organization_id,

        value: '',
        assets_id:''

            
    };
  },
  components: {
    HeaderTop
  },
  watch: {
    subordinate:function(subordinate){
      this.subordinate=subordinate
      this.getOffice()
      this.getFactory()
      this.getStory()
      this.getDorm()
      this.getOthers()
      this.getHouse()
    }
      
    
  },
  mounted() {
    this.getCompanyData();
    this.getOffice()
    this.getFactory()
    this.getStory()
    this.getDorm()
    this.getOthers()
    this.getHouse()
  },
  methods: {

    //获取耕地数据
    getOffice(){
      	this.$http
        		.post("app_module2/getAssetsJointStockCompany",{
                torganization_id:this.subordinate,
                nowPage:'1',
                type:'1',
                assets_category:'101'

        				}).then(result => {
            
           
            if(result.data.code==200){

            
            this.officeData=result.data.data.dataList
           
            
        
					}
			})
    },
    //获取林地数据
     getFactory(){
      	this.$http
        		.post("app_module2/getAssetsJointStockCompany",{
                torganization_id:this.subordinate,
                nowPage:'1',
                type:'1',
                assets_category:'103'

        				}).then(result => {
            
           
            if(result.data.code==200){

              
             
              
            this.factoryData=result.data.data.dataList
          
            // console.log(this.officeData);
          
					}
			})
    },
    //获取园地数据
     getStory(){
      	this.$http
        		.post("app_module2/getAssetsJointStockCompany",{
                torganization_id:this.subordinate,
                nowPage:'1',
                type:'1',
                assets_category:'115'

        				}).then(result => {
            
           
            if(result.data.code==200){

              
             
              
            this.storyData=result.data.data.dataList
          
            // console.log(this.officeData);
          
					}
			})
    },
    //获取草地数据
    getDorm(){
      	this.$http
        		.post("app_module2/getAssetsJointStockCompany",{
                torganization_id:this.subordinate,
                nowPage:'1',
                type:'1',
                assets_category:'116'

        				}).then(result => {
            
           
            if(result.data.code==200){

              
             
              
            this.dormData=result.data.data.dataList
          
            // console.log(this.officeData);
          
					}
			})
    },
    //获取商服数据
     getOthers(){
      	this.$http
        		.post("app_module2/getAssetsJointStockCompany",{
                torganization_id:this.subordinate,
                nowPage:'1',
                type:'1',
                assets_category:'109'

        				}).then(result => {
            
           
            if(result.data.code==200){

              
             
              
            this.othersData=result.data.data.dataList
          
            // console.log(this.officeData);
          
					}
			})
    },
    //获取住宅用地
     getHouse(){
      	this.$http
        		.post("app_module2/getAssetsJointStockCompany",{
                torganization_id:this.subordinate,
                nowPage:'1',
                type:'1',
                assets_category:'106'

        				}).then(result => {
            
           
            if(result.data.code==200){

              
             
              
            this.houseData=result.data.data.dataList
          
         
          
					}
			})
    },

    //获取公司数据
    getCompanyData(){
				this.$http
        		.post("app_module2/getStreetStockCompany",{
          			torganization_id:this.id
        				}).then(result => {
            
            
            if(result.data.code==200){
             this.companyData=result.data.data.pageList
    
					}
			})
      },
      // 去详情页
    toDetail(assets_id) {
       this.$router.push({
        path: "/resourceDetail",
        query: {
          assets_id: assets_id
        
        }
      });
    },
  },
};
</script>


<style lang='less' scoped>

.fixedAssets {
  width: 100%;
 
li{
  list-style: none;
}

  .zschoose{
    padding-left: .3rem;
    padding-right: .2rem;
  }
  .zstext-content {
     border-top: 0.2rem solid #e9e9e9;
    .choose{
      width: 100%;
    }
    // padding-left: 0.1rem;
    .zs-select {
      width: 100%;
      background-color: pink;
    }
    .zsul {
      padding-left: 0.3rem;
     
    }
  }
}
.zsli{
    border-bottom: .2rem solid #e9e9e9;
}
.zsh1{
   margin-top: .2rem;
    margin-bottom: .3rem;
     font-size: 18px;
    font-weight: normal;
}
.zs-last{
   
    border-top: 1px solid grey;
}
.zstwo{
    margin-bottom: .2rem;
    font-size: 14px;
}
.zsfont{
  font-size: 16px;
   margin-bottom: .2rem;
}
//  .zsmytabs/deep/.el-tabs__item{
//    font-size: .31rem;
 
// }
.choose/deep/input.el-input__inner{
  font-size: 18px;
}
li.el-select-dropdown__item{
   font-size: 18px;
}


</style>
